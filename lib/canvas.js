...
export const handleCanvasMouseDown = ({ options, isDrawing, canvas }) => {
  isDrawing = true;
  ...
  const rect = new fabric.Rect({
    ...
    height: 0,
    fill: "transparent",
    stroke: "blue", // Customize the stroke color
    strokeWidth: 2, // Customize the stroke width
  });

  canvas.add(rect);

  rect.on("mouse:move", (options) => {
    if (rect) {
      rect.set({
...
export const handleCanvasMouseUp = ({ isDrawing }) => {
