...
export class Annotation {
  ...
  #addOrRemoveFilter() {
    this.#darkFilter = new fabric.Rect({
      absolutePositioned: true,
      left: this.img.left,
      top: this.img.top,
      width: this.width * this.img.scaleX,
    ...
    if (
      this.canvas.getObjects().filter((arr) => arr.type == "rect").length > 0
    ) {
      this.canvas.add(this.#darkFilter);
    } else {
      this.canvas.remove(this.#darkFilter);
    }
  }

  #mouseDown() {
    this.canvas.on("mouse:down", (options) => {
      if (options.target !== this.img || options.target?.type === "rect") {
        this.#selected = options.target?.id;
        return;
      }
      this.#selected = null;
  ...
  #mouseMove() {
    this.canvas.on("mouse:move", (options) => {
      if (this.#rect) {
        ...
        this.#darkFilter.clipPath = this.#group;
